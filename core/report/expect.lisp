...
(defun make-expect (failed pred form expected lhs rhs)
  (make-instance 'expect :failed failed :predicate pred :form form :expected expected :lhs lhs :rhs rhs))


(defmethod report:print-report ((self expect) indent)
  (when (failed self)
    (with-accessors ((predicate predicate) (form form) (expected expected)) self
